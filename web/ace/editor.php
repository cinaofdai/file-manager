...
$root_path = $_SESSION['workspace'];
$curr_file = __FILE__; //默认编辑当前文件
$curr_file_path = str_replace(dirname(__FILE__), '', __FILE__);
$pwd = "258fb7379f3168e1a16d185d7b6cdfb5"; //密码初始化默认值为 false
$ace = 'ace.js'; //编辑器核心js
...
if ( is_logged() && is_ajax() && isset($_POST['create']) ) {
	...
	if ( $_POST['create'] && $target ) {
        ...
        $new_path = str_replace($root_path, '', $full_path);
...
if ( is_logged() && is_ajax() && isset($_POST['rename']) ) {
	...
	if ( $_POST['rename'] ) {
	    ...
	    if ( $base_name ) {
	        ...
	        $new_path = str_replace($root_path, '', $rename);
	    }
	}
...
$tree = '<ul id="dir_tree">
	<li class="dir" path="/" onclick="load()">ROOT'.list_dir($root_path, 'html').'</li>
</ul>';
